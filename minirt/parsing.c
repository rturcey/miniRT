...
int	error_msg(int n, int line)
{
	...
	//si le message est entre 5 et n, lancer fonction de free et de close
	return (-1);
}
...
int		ft_parser(char **buff, t_object **objs, t_p *p)
{
	if (parse_resolution(buff, p) == -1)
		return (-1);
	if (parse_cameras(buff, objs[1], p) == -1)
		return (-1);
	if (parse_lights(buff, objs[2], p) == -1)
		return (-1);
	if (parse_ambient(buff, p) == -1)
		return (-1);
	if (parse_effects(buff, p) == -1)
		return (-1);
	if (parse_objects(buff, objs[0], p) == -1)
		return (-1);
	return (0);
}
...
int	main(int argc, char **argv)
{
	...
	if (check_types(buf) == -1)
		return (-1);
	ft_parser(buf, objs, &p);
	p.o = objs[0];
	p.r = objs[1];
	p.l = objs[2];
	return (0);
	// prevoir le cas -save
	// msgs d'erreur dans parse-line
}