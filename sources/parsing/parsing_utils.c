...
double	ft_atod(char *str, int *i, double dot)
{
	double	number;
	double	sign;
	...
	while ((str[*i] >= '0' && str[*i] <= '9') || str[*i] == '.')
	{
		...
		else if ((number = number + (str[*i] - '0') / dot) >= 0)
			dot *= 10;
		(*i)++;
	}
...
int		parse_vecrot(char *v, t_vector *r, int *i)
{
	while ((v[*i] >= 9 && v[*i] <= 13) || v[*i] == 32)
		(*i)++;
	if ((r->x = ft_atod(v, i, 1)) == 1E99 || (r->x < -1 || r->x > 1))
		return (-1);
	while (v[*i] == '-' || ft_isdigit(v[*i]) || v[*i] == '.')
		(*i)++;
	if (v[*i] != ',')
		return (-1);
	(*i)++;
	if ((r->y = ft_atod(v, i, 1)) == 1E99 || (r->y < -1 || r->y > 1))
		return (-1);
	while (v[*i] == '-' || ft_isdigit(v[*i]) || v[*i] == '.')
		(*i)++;
	if (v[*i] != ',')
		return (-1);
	(*i)++;
	if ((r->z = ft_atod(v, i, 1)) == 1E99 || (r->z < -1 || r->z > 1))
		return (-1);
	while (v[*i] == '-' || ft_isdigit(v[*i]) || v[*i] == '.')
		(*i)++;
	return (0);
